...
(defun cp-reg-with-val (cpu reg val)
  (let ((res (if (< reg val) (- (+ reg #x100) val) (- reg val))))
    (format t "~A - ~A~%" reg val)
    (setf (gbflags-z (gbcpu-flags cpu)) (if (= (logand res #xff) #x00) #x01 #x00)
          (gbflags-n (gbcpu-flags cpu)) 1
...
(setf (aref ops #x06) (make-instruction :opcode #x06 :bytes 2 :cycles 8 :asm '(:ld "B,u8")
                                        :fun (lambda (cpu)
                                                (let ((b (aref memory (+ (gbcpu-pc cpu) 1))))
                                                  (setf (gbcpu-b cpu) b)
                                                  (incf (gbcpu-pc cpu) 2)))))
(setf (aref ops #x07) (make-instruction :opcode #x07 :bytes 1 :cycles 4 :asm '(:rlca)
                                        :fun (lambda (cpu)
...
(setf (aref ops #x0e) (make-instruction :opcode #x0e :bytes 2 :cycles 8 :asm '(:ld "C,u8")
                                        :fun (lambda (cpu)
                                                (let ((b (aref memory (+ (gbcpu-pc cpu) 1))))
                                                  (setf (gbcpu-c cpu) b)
                                                  (incf (gbcpu-pc cpu) 2)))))
(setf (aref ops #x0f) (make-instruction :opcode #x0f :bytes 1 :cycles 4 :asm '(:rrca)
                                        :fun (lambda (cpu)
...
(setf (aref ops #x2e) (make-instruction :opcode #x2e :bytes 2 :cycles 8 :asm '(:ld "L,u8")
                                        :fun (lambda (cpu)
                                                (let ((b (aref memory (+ (gbcpu-pc cpu) 1))))
                                                  (setf (gbcpu-l cpu) b)
                                                  (incf (gbcpu-pc cpu) 2)))))
(setf (aref ops #x2f) (make-instruction :opcode #x2f :bytes 1 :cycles 4 :asm '(:cpl)
                                        :fun (lambda (cpu)
...
(setf (aref ops #x36) (make-instruction :opcode #x36 :bytes 2 :cycles 12 :asm '(:ld "(HL),u8")
                                        :fun (lambda (cpu)
                                                (let ((b (aref memory (+ (gbcpu-pc cpu) 1)))
                                                  ...
                                                  (incf (gbcpu-pc cpu) 2)))))
(setf (aref ops #x37) (make-instruction :opcode #x37 :bytes 1 :cycles 4 :asm '(:scf)
                                        :fun (lambda (cpu)
                                                  (setf (gbflags-n (gbcpu-flags cpu)) #x00
...
(setf (aref ops #x3e) (make-instruction :opcode #x3e :bytes 2 :cycles 8 :asm '(:ld "A,u8")
                                        :fun (lambda (cpu)
                                                (let ((b (aref memory (+ (gbcpu-pc cpu) 1))))
                                                  (setf (gbcpu-a cpu) b)
                                                  (incf (gbcpu-pc cpu) 2)))))
(setf (aref ops #x3f) (make-instruction :opcode #x3f :bytes 1 :cycles 4 :asm '(:ccf)
                                        :fun (lambda (cpu)
                                                  (setf (gbflags-n (gbcpu-flags cpu)) #x00
                                                        (gbflags-h (gbcpu-flags cpu)) #x00
...
;; Arithmentic
;; ADDs
(setf (aref ops #x80) (make-instruction :opcode #x80 :bytes 1 :cycles 4 :asm '(:add "A,B")
                                        :fun (lambda (cpu)
                                               (let ((res (add cpu (gbcpu-a cpu) (gbcpu-b cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x81) (make-instruction :opcode #x81 :bytes 1 :cycles 4 :asm '(:add "A,C")
                                        :fun (lambda (cpu)
                                               (let ((res (add cpu (gbcpu-a cpu) (gbcpu-c cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x82) (make-instruction :opcode #x82 :bytes 1 :cycles 4 :asm '(:add "A,D")
                                        :fun (lambda (cpu)
                                               (let ((res (add cpu (gbcpu-a cpu) (gbcpu-d cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x83) (make-instruction :opcode #x83 :bytes 1 :cycles 4 :asm '(:add "A,E")
                                        :fun (lambda (cpu)
                                               (let ((res (add cpu (gbcpu-a cpu) (gbcpu-e cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x84) (make-instruction :opcode #x84 :bytes 1 :cycles 4 :asm '(:add "A,H")
                                        :fun (lambda (cpu)
                                               (let ((res (add cpu (gbcpu-a cpu) (gbcpu-h cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x85) (make-instruction :opcode #x85 :bytes 1 :cycles 4 :asm '(:add "A,L")
                                        :fun (lambda (cpu)
...
(setf (aref ops #x86) (make-instruction :opcode #x86 :bytes 1 :cycles 8 :asm '(:add "A,(HL)")
                                        :fun (lambda (cpu)
                                                  ...
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #x87) (make-instruction :opcode #x87 :bytes 1 :cycles 4 :asm '(:add "A,A")
                                        :fun (lambda (cpu)
...
;; ADCs
(setf (aref ops #x88) (make-instruction :opcode #x88 :bytes 1 :cycles 4 :asm '(:adc "A,B")
                                        :fun (lambda (cpu)
                                               (let ((res (adc cpu (gbcpu-a cpu) (gbcpu-b cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x89) (make-instruction :opcode #x89 :bytes 1 :cycles 4 :asm '(:adc "A,C")
                                        :fun (lambda (cpu)
                                               (let ((res (adc cpu (gbcpu-a cpu) (gbcpu-c cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x8a) (make-instruction :opcode #x8a :bytes 1 :cycles 4 :asm '(:adc "A,D")
                                        :fun (lambda (cpu)
                                               (let ((res (adc cpu (gbcpu-a cpu) (gbcpu-d cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x8b) (make-instruction :opcode #x8b :bytes 1 :cycles 4 :asm '(:adc "A,E")
                                        :fun (lambda (cpu)
                                               (let ((res (adc cpu (gbcpu-a cpu) (gbcpu-e cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x8c) (make-instruction :opcode #x8c :bytes 1 :cycles 4 :asm '(:adc "A,H")
                                        :fun (lambda (cpu)
                                               (let ((res (adc cpu (gbcpu-a cpu) (gbcpu-h cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x8d) (make-instruction :opcode #x8d :bytes 1 :cycles 4 :asm '(:adc "A,L")
                                        :fun (lambda (cpu)
                                               (let ((res (adc cpu (gbcpu-a cpu) (gbcpu-l cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x8e) (make-instruction :opcode #x8e :bytes 1 :cycles 8 :asm '(:adc "A,(HL)")
                                        :fun (lambda (cpu)
                                                  (setf (gbcpu-a cpu) (adc cpu (gbcpu-a cpu) (get-byte-from-hl-address cpu)))
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #x8f) (make-instruction :opcode #x8f :bytes 1 :cycles 4 :asm '(:adc "A,A")
                                        :fun (lambda (cpu)
...
(setf (aref ops #x90) (make-instruction :opcode #x90 :bytes 1 :cycles 4 :asm '(:sub "A,B")
                                        :fun (lambda (cpu)
                                               (let ((res (sub cpu (gbcpu-a cpu) (gbcpu-b cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x91) (make-instruction :opcode #x91 :bytes 1 :cycles 4 :asm '(:sub "A,C")
                                        :fun (lambda (cpu)
                                               (let ((res (sub cpu (gbcpu-a cpu) (gbcpu-c cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x92) (make-instruction :opcode #x92 :bytes 1 :cycles 4 :asm '(:sub "A,D")
                                        :fun (lambda (cpu)
                                               (let ((res (sub cpu (gbcpu-a cpu) (gbcpu-d cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x93) (make-instruction :opcode #x93 :bytes 1 :cycles 4 :asm '(:sub "A,E")
                                        :fun (lambda (cpu)
                                               (let ((res (sub cpu (gbcpu-a cpu) (gbcpu-e cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x94) (make-instruction :opcode #x94 :bytes 1 :cycles 4 :asm '(:sub "A,H")
                                        :fun (lambda (cpu)
                                               (let ((res (sub cpu (gbcpu-a cpu) (gbcpu-h cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x95) (make-instruction :opcode #x95 :bytes 1 :cycles 4 :asm '(:sub "A,L")
                                        :fun (lambda (cpu)
                                               (let ((res (sub cpu (gbcpu-a cpu) (gbcpu-l cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x96) (make-instruction :opcode #x96 :bytes 1 :cycles 8 :asm '(:sub "A,(HL)")
                                        :fun (lambda (cpu)
                                                  (setf (gbcpu-a cpu) (sub cpu (gbcpu-a cpu) (get-byte-from-hl-address cpu)))
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #x97) (make-instruction :opcode #x97 :bytes 1 :cycles 4 :asm '(:sub "A,A")
                                        :fun (lambda (cpu)
...
;; SBCs
(setf (aref ops #x98) (make-instruction :opcode #x98 :bytes 1 :cycles 4 :asm '(:sbc "A,B")
                                        :fun (lambda (cpu)
                                               (let ((res (sbc cpu (gbcpu-a cpu) (gbcpu-b cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x99) (make-instruction :opcode #x99 :bytes 1 :cycles 4 :asm '(:sbc "A,C")
                                        :fun (lambda (cpu)
                                               (let ((res (sbc cpu (gbcpu-a cpu) (gbcpu-c cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x9a) (make-instruction :opcode #x9a :bytes 1 :cycles 4 :asm '(:sbc "A,D")
                                        :fun (lambda (cpu)
                                               (let ((res (sbc cpu (gbcpu-a cpu) (gbcpu-d cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x9b) (make-instruction :opcode #x9b :bytes 1 :cycles 4 :asm '(:sbc "A,E")
                                        :fun (lambda (cpu)
                                               (let ((res (sbc cpu (gbcpu-a cpu) (gbcpu-e cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x9c) (make-instruction :opcode #x9c :bytes 1 :cycles 4 :asm '(:sbc "A,H")
                                        :fun (lambda (cpu)
                                               (let ((res (sbc cpu (gbcpu-a cpu) (gbcpu-h cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x9d) (make-instruction :opcode #x9d :bytes 1 :cycles 4 :asm '(:sbc "A,L")
                                        :fun (lambda (cpu)
                                               (let ((res (sbc cpu (gbcpu-a cpu) (gbcpu-l cpu))))
                                                 (setf (gbcpu-a cpu) res)
                                                 (incf (gbcpu-pc cpu))))))
(setf (aref ops #x9e) (make-instruction :opcode #x9e :bytes 1 :cycles 8 :asm '(:sbc "A,(HL)")
                                        :fun (lambda (cpu)
                                                  (setf (gbcpu-a cpu) (sbc cpu (gbcpu-a cpu) (get-byte-from-hl-address cpu)))
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #x9f) (make-instruction :opcode #x9f :bytes 1 :cycles 4 :asm '(:sbc "A,A")
                                        :fun (lambda (cpu)
...
;; ANDs
(setf (aref ops #xa0) (make-instruction :opcode #xa0 :bytes 1 :cycles 4 :asm '(:and "A,B")
                                        :fun (lambda (cpu)
                                               (let ((res (and-op cpu (gbcpu-a cpu) (gbcpu-b cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xa1) (make-instruction :opcode #xa1 :bytes 1 :cycles 4 :asm '(:and "A,C")
                                        :fun (lambda (cpu)
                                               (let ((res (and-op cpu (gbcpu-a cpu) (gbcpu-c cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xa2) (make-instruction :opcode #xa2 :bytes 1 :cycles 4 :asm '(:and "A,D")
                                        :fun (lambda (cpu)
                                               (let ((res (and-op cpu (gbcpu-a cpu) (gbcpu-d cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xa3) (make-instruction :opcode #xa3 :bytes 1 :cycles 4 :asm '(:and "A,E")
                                        :fun (lambda (cpu)
                                               (let ((res (and-op cpu (gbcpu-a cpu) (gbcpu-e cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xa4) (make-instruction :opcode #xa4 :bytes 1 :cycles 4 :asm '(:and "A,H")
                                        :fun (lambda (cpu)
                                               (let ((res (and-op cpu (gbcpu-a cpu) (gbcpu-h cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xa5) (make-instruction :opcode #xa5 :bytes 1 :cycles 4 :asm '(:and "A,L")
                                        :fun (lambda (cpu)
                                               (let ((res (and-op cpu (gbcpu-a cpu) (gbcpu-l cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xa6) (make-instruction :opcode #xa6 :bytes 1 :cycles 8 :asm '(:and "A,(HL)")
                                        :fun (lambda (cpu)
                                               (let ((res (and-op cpu (gbcpu-a cpu) (get-byte-from-hl-address cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xa7) (make-instruction :opcode #xa7 :bytes 1 :cycles 4 :asm '(:and "A,A")
                                        :fun (lambda (cpu)
...
;; XORs
(setf (aref ops #xa8) (make-instruction :opcode #xa8 :bytes 1 :cycles 4 :asm '(:xor "A,B")
                                        :fun (lambda (cpu)
...
(setf (aref ops #xa9) (make-instruction :opcode #xa9 :bytes 1 :cycles 4 :asm '(:xor "A,C")
                                        :fun (lambda (cpu)
                                               (let ((res (xor cpu (gbcpu-a cpu) (gbcpu-c cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xaa) (make-instruction :opcode #xaa :bytes 1 :cycles 4 :asm '(:xor "A,D")
                                        :fun (lambda (cpu)
                                               (let ((res (xor cpu (gbcpu-a cpu) (gbcpu-d cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xab) (make-instruction :opcode #xab :bytes 1 :cycles 4 :asm '(:xor "A,E")
                                        :fun (lambda (cpu)
                                               (let ((res (xor cpu (gbcpu-a cpu) (gbcpu-e cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xac) (make-instruction :opcode #xac :bytes 1 :cycles 4 :asm '(:xor "A,H")
                                        :fun (lambda (cpu)
...
(setf (aref ops #xb1) (make-instruction :opcode #xb1 :bytes 1 :cycles 4 :asm '(:or "A,C")
                                        :fun (lambda (cpu)
                                               (let ((res (or-op cpu (gbcpu-a cpu) (gbcpu-c cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xb2) (make-instruction :opcode #xb2 :bytes 1 :cycles 4 :asm '(:or "A,D")
                                        :fun (lambda (cpu)
                                               (let ((res (or-op cpu (gbcpu-a cpu) (gbcpu-d cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xb3) (make-instruction :opcode #xb3 :bytes 1 :cycles 4 :asm '(:or "A,E")
                                        :fun (lambda (cpu)
                                               (let ((res (or-op cpu (gbcpu-a cpu) (gbcpu-e cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xb4) (make-instruction :opcode #xb4 :bytes 1 :cycles 4 :asm '(:or "A,H")
                                        :fun (lambda (cpu)
                                               (let ((res (or-op cpu (gbcpu-a cpu) (gbcpu-h cpu))))
                                                  (setf (gbcpu-a cpu) res)
                                                  (incf (gbcpu-pc cpu))))))
(setf (aref ops #xb5) (make-instruction :opcode #xb5 :bytes 1 :cycles 4 :asm '(:or "A,L")
                                        :fun (lambda (cpu)
...
;; CPs
(setf (aref ops #xb8) (make-instruction :opcode #xb8 :bytes 1 :cycles 4 :asm '(:cp "A,B")
                                           :fun (lambda (cpu)
                                                  (cp-reg-with-val cpu (gbcpu-a cpu) (gbcpu-b cpu))
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #xb9) (make-instruction :opcode #xb9 :bytes 1 :cycles 4 :asm '(:cp "A,C")
                                           :fun (lambda (cpu)
                                                  (cp-reg-with-val cpu (gbcpu-a cpu) (gbcpu-c cpu))
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #xba) (make-instruction :opcode #xba :bytes 1 :cycles 4 :asm '(:cp "A,D")
                                           :fun (lambda (cpu)
                                                  (cp-reg-with-val cpu (gbcpu-a cpu) (gbcpu-d cpu))
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #xbb) (make-instruction :opcode #xbb :bytes 1 :cycles 4 :asm '(:cp "A,E")
                                           :fun (lambda (cpu)
                                                  (cp-reg-with-val cpu (gbcpu-a cpu) (gbcpu-e cpu))
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #xbc) (make-instruction :opcode #xbc :bytes 1 :cycles 4 :asm '(:cp "A,H")
                                           :fun (lambda (cpu)
                                                  (cp-reg-with-val cpu (gbcpu-a cpu) (gbcpu-h cpu))
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #xbd) (make-instruction :opcode #xbd :bytes 1 :cycles 4 :asm '(:cp "A,L")
                                           :fun (lambda (cpu)
...
(setf (aref ops #xbe) (make-instruction :opcode #xbe :bytes 1 :cycles 8 :asm '(:cp "A,(HL)")
                                           :fun (lambda (cpu)
                                                  ...
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #xbf) (make-instruction :opcode #xbf :bytes 1 :cycles 4 :asm '(:cp "A,A")
                                           :fun (lambda (cpu)
...
(setf (aref ops #xc1) (make-instruction :opcode #xc1 :bytes 1 :cycles 12 :asm '(:pop "BC")
                                        :fun (lambda (cpu)
                                                  ...
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #xc2) (make-instruction :opcode #xc2 :bytes 3 :cycles 16 :asm '(:jp "NZ,u16")
                                        :fun (lambda (cpu)
                                                (let ((addr (get-address-from-memory memory (+ (gbcpu-pc cpu) 1))))
                                                  (incf (gbcpu-pc cpu) 3)
                                                  (if (= (gbflags-z (gbcpu-flags cpu)) #x00)
...
(setf (aref ops #xf1) (make-instruction :opcode #xf1 :bytes 1 :cycles 12 :asm '(:pop "AF")
                                        :fun (lambda (cpu)
                                                  ...
                                                  (incf (gbcpu-pc cpu)))))
(setf (aref ops #xf2) (make-instruction :opcode #xf2 :bytes 1 :cycles 8 :asm '(:ld "A,(FF00+C)")
                                        :fun (lambda (cpu)
                                                (let ((addr (+ #xff00 (gbcpu-c cpu))))
                                                  (setf (gbcpu-a cpu) (aref memory addr))
                                                  (incf (gbcpu-pc cpu))))))
...
(defun emu-main (cpu)
  (loop for o = (aref memory (gbcpu-pc cpu))
        ...
        while instr do (format t "~X: ~A --> PC=~X~%" (instruction-opcode instr) (instruction-asm instr) (gbcpu-pc cpu))
...
; PASS (replace-memory-with-rom "~/repos/github/retrio/gb-test-roms/cpu_instrs/individual/06-ld r,r.gb")
;(replace-memory-with-rom "~/repos/github/retrio/gb-test-roms/cpu_instrs/individual/07-jr,jp,call,ret,rst.gb")
; PASS (replace-memory-with-rom "~/repos/github/retrio/gb-test-roms/cpu_instrs/individual/08-misc instrs.gb")
;(replace-memory-with-rom "~/repos/github/retrio/gb-test-roms/cpu_instrs/individual/09-op r,r.gb")
;(replace-memory-with-rom "~/repos/github/retrio/gb-test-roms/cpu_instrs/individual/10-bit ops.gb")
;(replace-memory-with-rom "~/repos/github/retrio/gb-test-roms/cpu_instrs/individual/11-op a,(hl).gb")
