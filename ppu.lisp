  ...
  (let* ((row (gbppu-cur-line ppu))
    ...
    (loop for col = 0 then (+ col 8)
      ...
      (let* ((yoffset (- row w-y))
        ...
        (render-tile-line ppu (gbppu-framebuffer ppu) row color-addr :start-x xoffset :is-background? t))))))
